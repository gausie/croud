...
\begin{document}
	...
	\label{sec:architecture}
		...
		In Figure \ref{fig:component-diagram}, the relationship between components in the project is described. The framework follows a server-client architecture. Each of these systems contain a set of subcomponents described in the following subsections.
...
exports.hasAuthorization = function(req, res, next) {
  ...
  next();
	...
	\begin{tabularx}{\textwidth}{ | X | X | X | X | X | X | }
		...
		\path{/users/:userId/memberships/:campaignId} & PUT &
		Join a user to a campaign &
		\textit{campaignId} & Id of the campaign to join &
		\textit{User} object.
		\\ \hline

		\path{/users/:userId/memberships/:campaignId} & DELETE &
		...
		\path{/auth/forgot} & POST &
		Request a password reset for a user &
		\textit{username} & The username of the user for whom the password is to be reset. &
		Message text
		\\ \hline


		\path{/auth/reset/:token} & POST &
		Reset a password from an email token &
		\textit{token}, \textit{newPassword}, \textit{verifyPassword} & The unique reset token, the new password, the new password repeated &
		\textit{User} object.
		\\ \hline

		\path{auth/signup}& POST &
		Create a new user account &
		\multicolumn{2}{ l | }{\textit{User} object.} &
		\textit{User} object.
		\\ \hline

		\path{auth/signin}& POST &
		Authenticate a user &
		\textit{username}, \textit{password} &
		The username, the password &
		\textit{User} object.
		\\ \hline

		\path{auth/signout} & GET &
		Sign a user out &
		\multicolumn{2}{ l | }{\textit{none}} &
		\textit{none}
		\\ \hline
	...
	\begin{tabularx}{\textwidth}{ | X | X | X | X | X | X | }
		...
		\textit{mine}, \textit{includeClosed} & Get only the own user's campaigns, include campaigns that have closed (duration is over). &
		Array of \textit{Campaign} objects.
		\\ \hline

		\path{/campaigns} & POST &
		Create a new Campaign &
		\multicolumn{2}{ l | }{\textit{Campaign} object.} &
		\textit{Campaign} object.
		\\ \hline

		\path{/campaigns/:campaignId} & GET &
